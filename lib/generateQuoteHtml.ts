...
export function generateQuoteHtml(d: QuoteHtmlData): string {
  ...
  const tableRows = d.services.map((s) => `
    <tr>
      <td style="padding:9px 12px;border-bottom:1px solid #f3e5f5;vertical-align:top;">
      ...
      <td style="padding:9px 12px;text-align:center;border-bottom:1px solid #f3e5f5;font-size:12px;vertical-align:top;">${s.quantity}</td>
      ${!d.hidePrice ? `
      <td style="padding:9px 12px;text-align:right;border-bottom:1px solid #f3e5f5;font-size:12px;vertical-align:top;">${money(s.unitPrice)}</td>
      <td style="padding:9px 12px;text-align:right;border-bottom:1px solid #f3e5f5;font-size:12px;font-weight:600;vertical-align:top;">${money(s.quantity * s.unitPrice)}</td>
      ` : ''}
    </tr>`).join('');
  ...
  const menuItems = d.services
    .filter((s) => s.name)
    .map((s) => `
      <div style="margin-bottom:22px;padding-bottom:22px;border-bottom:1px solid #f3e5f5;text-align:center;">
        <h3 style="font-size:15px;font-weight:bold;color:#9c27b0;margin:0 0 5px;letter-spacing:0.3px;">${s.name}</h3>
        ${s.description
          ? `<p class="svc-desc" style="font-size:12px;color:#777;font-style:italic;margin:0;line-height:1.55;">${s.description}</p>`
          : ''}
      </div>`).join('');

  // ── HEADER gradient shared CSS ─────────────────────────────────────────────
  const headerGradient = 'background:linear-gradient(135deg,#6a1080 0%,#9c27b0 60%,#ab47bc 100%);';

  return `
<!-- ═══════════════════════ PAGE 1 — DEVIS FINANCIER ═══════════════════════ -->
<div style="font-family:'Georgia',serif;color:#1a1a1a;line-height:1.65;min-height:257mm;">
  ...
  <div style="display:flex;gap:14px;margin-top:18px;margin-bottom:16px;">
    <div style="flex:1;background:#faf5ff;border:1px solid #e9d5ff;border-radius:8px;padding:13px;">
      <p style="font-size:9px;font-weight:bold;color:#9c27b0;text-transform:uppercase;letter-spacing:1.5px;margin:0 0 5px;">CLIENT</p>
  ...
  <!-- ── Intro ── -->
  <div style="margin-bottom:16px;padding:11px 14px;border-left:4px solid #9c27b0;background:#fafafa;border-radius:0 5px 5px 0;">
  ...
  <div style="display:flex;justify-content:flex-end;margin-bottom:20px;">
    <div style="min-width:195px;background:#faf5ff;border:1px solid #e9d5ff;border-radius:8px;padding:12px;">
      ...
      <div style="display:flex;justify-content:space-between;margin-bottom:10px;font-size:12px;color:#666;padding-bottom:10px;border-bottom:1px solid #e9d5ff;">
        <span>TVA (${d.vatRate}%)</span><span>${money(vat)}</span>
      </div>
      <div style="display:flex;justify-content:space-between;font-size:15px;font-weight:bold;color:#9c27b0;">
  ...
  <div style="margin-bottom:16px;">
    <p style="font-size:9px;font-weight:bold;color:#9c27b0;text-transform:uppercase;letter-spacing:1.5px;margin:0 0 5px;">REMARQUES</p>
...
<div style="font-family:'Georgia',serif;color:#1a1a1a;line-height:1.7;min-height:257mm;">
